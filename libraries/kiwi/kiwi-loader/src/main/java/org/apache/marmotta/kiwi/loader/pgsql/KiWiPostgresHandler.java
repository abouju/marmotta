...
public class KiWiPostgresHandler extends KiWiBatchHandler implements RDFHandler {
    ...
    protected void flushBacklogInternal() throws SQLException {
        try {
            // flush out nodes
            PGCopyOutputStream nodesOut = new PGCopyOutputStream(PGCopyUtil.getWrappedConnection(connection.getJDBCConnection()), "COPY nodes(id,ntype,svalue,dvalue,ivalue,tvalue,tzoffset,bvalue,ltype,lang,createdAt,gvalue) FROM STDIN (FORMAT csv)");
