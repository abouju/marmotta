...
// auto-register for SPARQL environment
	...
	System.out.println("DistanceFunction.java args "+args.length);
	System.out.println("DistanceFunction.java FN_GEOSPARQL.meter.toString() "+FN_GEOSPARQL.meter.toString());
	System.out.println("DistanceFunction.java FN_GEOSPARQL.degree.toString() "+FN_GEOSPARQL.degree.toString());
	System.out.println("DistanceFunction.java FN_GEOSPARQL.radian.toString() "+FN_GEOSPARQL.radian.toString());
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 3) {
                String geom1 = args[0];
                String geom2 = args[1];
                String SRID_default = "4326";
		System.out.println("DistanceFunction.java args[2] "+args[2]);
                /*
		    ...
		    /* DistanceSpheroid todo ST_DistanceSpheroid */
            ...
            { // With SRID TODO          
             if (args.length == 4) {
                 ...
                 */
                if (args[0].contains("POINT") || args[0].contains("MULTIPOINT") || args[0].contains("LINESTRING") || args[0].contains("MULTILINESTRING") || args[0].contains("POLYGON") || args[0].contains("MULTIPOLYGON") || args[0].contains("ST_AsText")) {
                    geom1 = String.format("ST_GeomFromText(%s,%s)", args[0], SRID);
                }
                if (args[1].contains("POINT") || args[1].contains("MULTIPOINT") || args[1].contains("LINESTRING") || args[1].contains("MULTILINESTRING") || args[1].contains("POLYGON") || args[1].contains("MULTIPOLYGON") || args[1].contains("ST_AsText")) {
                    geom2 = String.format("ST_GeomFromText(%s,%s)", args[1], SRID);
                }
		/* more control to add */
                if (args[2].equalsIgnoreCase("'" + FN_GEOSPARQL.meter.toString() + "'") || args[2].equalsIgnoreCase("'" + FN_GEOSPARQL.metre.toString() + "'")) {
                    return String.format("ST_Distance( ST_Transform( %s ,SRID), ST_Transform( %s ,SRID))", geom1, geom2);
                }
                if (args[2].equalsIgnoreCase("'" + FN_GEOSPARQL.degree.toString() + "'")) {
                    return String.format("ST_Distance(%s, %s)", geom1, geom2);
                }
                ...
                if (args[2].equalsIgnoreCase("'" + FN_GEOSPARQL.unit.toString() + "'")) {
                    return String.format("ST_Distance(%s, %s)", geom1, geom2);
                }
             }
            }
